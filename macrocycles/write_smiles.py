...
### This code has been modified from its original version, originally accessed through GitHub: https://github.com/pckroon/pysmiles
...
def format_atom(molecule, node_key, default_element='*'):
    ...
    if name == "CX":
        if aromatic:
            ...
            name = "*"
    #print(node.get('aromatic'))
    #print("STER:" , node.get('stereo'))
    ...
    if name != "n" and stereo not in ["R", "S"]:
        hcountstr = ""
    ...
    if stereo == "R":
        ...
        #print("STER MISS: ", stereo)
        stereostr = ""
    #retval = fmt.format(isotope=isotope, name=name, stereo='', hcount=hcountstr,
    #                  charge=chargestr, class_=class_)
    retval = fmt.format(isotope=isotope, name=name, stereo=stereostr, hcount=hcountstr,
                      charge=chargestr, class_=class_)
...
def write_smiles(molecule, default_element='*', start=None):
    ...
    order_to_symbol = {0: '.', 1: '-', 1.5: ':', 2: '=', 3: '#', 4: '$', 12: ""}
    ...
    while to_visit:
        ...
        if current in predecessors:
            ...
            if _write_edge_symbol(molecule, previous, current):
                order = int(molecule.edges[previous, current].get('order', 1))
                #if order == 12:
                #    order = 1
        ...
        if current in atom_to_ring_idx:
            ...
            for ring_idx in ring_idxs:
                ...
                if _write_edge_symbol(molecule, *ring_bond) and new_marker:
                    order = int(molecule.edges[ring_bond].get('order', 1))
                    if order == 12:
                        order = 1
                    smiles += order_to_symbol[order]
